...
class TestMessageHeader(unittest.TestCase):
    ...
    def test_wrong_versions(self):
        values = [0, 'a', -1, ""]
        for value in values:
            self.my_msg_recv['header']['version'] = value
            self.assertEqual(self.message.validate_header(self.my_msg_recv['header']), False)

    def test_rigt_versions(self):
        values = [1, '1']
        for value in values:
            self.my_msg_recv['header']['version'] = value
            self.assertEqual(self.message.validate_header(self.my_msg_recv['header']), True)

    # Test message.header.id
    def test_wrong_ids(self):
        values = [256, 'a', -1, ""]
        for value in values:
            self.my_msg_recv['header']['id'] = value
            self.assertEqual(self.message.validate_header(self.my_msg_recv['header']), False)

    def test_rigt_ids(self):
        values = [0, 1, '1']
        for value in values:
            self.my_msg_recv['header']['id'] = value
            self.assertEqual(self.message.validate_header(self.my_msg_recv['header']), True)
    ...
    def test_wrong_payloads(self):
        values = [256, 'a', -1, ""]
        for value in values:
            self.my_msg_recv['header']['payload'] = value
            self.assertEqual(self.message.validate_header(self.my_msg_recv['header']), False)

    def test_rigt_payloads(self):
        values = [0, 1, '1']
        for value in values:
            self.my_msg_recv['header']['payload'] = value
            self.assertEqual(self.message.validate_header(self.my_msg_recv['header']), True)
    ...
    def test_wrong_timings(self):
        values = [256, 'a', -1, ""]
        for value in values:
            self.my_msg_recv['header']['timing'] = value
            self.assertEqual(self.message.validate_header(self.my_msg_recv['header']), False)

    def test_rigt_timings(self):
        values = [0, 1, '1']
        for value in values:
            self.my_msg_recv['header']['timing'] = value
            self.assertEqual(self.message.validate_header(self.my_msg_recv['header']), True)
...
class TestMessageBody(unittest.TestCase):
    ...
    # Test message.body.suggested_id
    def test_wrong_suggested_ids(self):
        values = [0, 'a', -1, "", 256]
        for value in values:
            self.my_msg_recv['body']['suggested_id'] = value
            self.assertEqual(self.message.validate_body(self.my_msg_recv['body']), False)

    def test_rigt_suggested_ids(self):
        values = [1, '1', 100, 255]
        for value in values:
            self.my_msg_recv['body']['suggested_id'] = value
            self.assertEqual(self.message.validate_body(self.my_msg_recv['body']), True)

    # TODO: all other body possibilities
